...
const ChooseComponent = ({character}:{character:characterSheet2}) =>{
    ...
    return (<><Link href={`/character/sheet/${character.id}/${character.type}`} >
        ...
        <span className="absolute right-0 bottom-0 text-black hover:bg-gray-400 w-8 h-8 z-50 rounded-full transform scale-75 flex items-center justify-center" onClick={deleteSheet}>
        <IconButtonMenu>
          <MenuItem  onClick={deleteSheet} disableGutters>
